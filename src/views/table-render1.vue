...
<script>
  ...
  export default {
    ...
    data () {
      return {
        columns: [
          {
            title: '姓名',
            //name 这一列仍然是使用 Functional Render，只不过 Render 的是一个预先定义好的 slot-scope 模板
    ...
    mounted () {
      /**注意的是，示例中的 `data` 默认是空数组，而在 mounted 里才赋值的，
       * 是因为这样定义的 slot-scope，初始时读取 `this.$refs.table.$scopedSlots` 是读不到的，
       * 会报错，当没有数据时，也就不会去渲染，也就避免了报错 */
      this.data = [
        {
          name: '王小明',
          age: 18,
